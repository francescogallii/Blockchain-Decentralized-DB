...
class CryptoUtils {
  ...
  // Build the consistent input string for block hashing
  static buildHashInput(blockData) {
    return [
      ...
      new Date(blockData.created_at).toISOString(),
  ...
  // Generate a secure random nonce for mining
  static generateNonce = () => crypto.randomBytes(8).readBigUInt64BE(0);
